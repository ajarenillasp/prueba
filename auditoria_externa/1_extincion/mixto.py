...
def con_aire_y_tierra(c, flota, E, horizonte_min, lat, doy,
    ...
    while t < horizonte_min:
        ...
        # Condición de parada correcta: el fuego está contenido cuando NO HAY
        # propagación posible, no cuando todas las celdas están "alcanzadas".
        # Se verifica que no queden celdas por quemar (V.INF) que sean accesibles
        # desde el perímetro activo. Una celda con V.INF puede ser inaccesible
        # (rodeada de barreras) o accesible (aún hay camino).
        _perimetro_activo = False
        for k in range(N):
            a = arr[k]
            if a == V.INF or a > t:
                continue
            # Esta celda ya ardió, ¿tiene vecinos sin quemar accesibles?
            i, j = divmod(k, nc)
            for di in (-1, 0, 1):
                for dj in (-1, 0, 1):
                    if not di and not dj:
                    ...
                    if b == V.INF and not held[nb]:
                        # Hay un vecino sin quemar y sin contener -> el fuego puede seguir
                        _perimetro_activo = True
                        break
                if _perimetro_activo:
                    break
            if _perimetro_activo:
                break
