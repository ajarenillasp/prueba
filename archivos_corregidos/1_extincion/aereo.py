...
def con_aire(c, flota, horizonte_min, lat, doy, iteraciones=3, bt=None):
    ...
    # F5 (2026-08-19) CORREGIDO: el riego se ACUMULA entre iteraciones, no se reinicia.
    # Antes `ws`/`we` estaban DENTRO del bucle (líneas 512-513 originales), lo que hacía
    # que la misma flota soltara su carga tantas veces como iteraciones — agua fantasma.
    # Ahora se inicializan UNA VEZ antes del bucle para acumular el efecto real del agua.
    ws = [V.INF] * N
    we = [V.INF] * N
    ...
    for paso in range(iteraciones):
        ...
        # El calendario se recalcula sobre el fuego actual, pero el agua acumulada
        # se conserva. Esto permite que múltiples iteraciones refinen dónde cae el
        # agua sin duplicar el volumen total descargado.
